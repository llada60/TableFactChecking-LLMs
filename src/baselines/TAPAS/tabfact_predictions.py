...
def predict(table_data, queries, args, verbose=False):
  table = [list(map(lambda s: s.strip(), row.split("#"))) for row in table_data.strip().split("\n") if row.strip()]
...
for key, value in pbar:
    ...
    # print(value)
    # questions = value["statement"]
    # labels = value["label"]
    # table_csv = value["table"]
    # entity = value["table_title"]
    ...
    pbar.set_description(f"Tapas acc so far: {correct}/{total}={correct/total*100:.2f}%")
